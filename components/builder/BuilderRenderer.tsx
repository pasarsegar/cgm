  ...
  switch (block.type) {
    ...
    case 'text':
      ...
      if (block.content.noContainer) {
        return <div style={textStyle}>{parse(block.content.html || '')}</div>;
      }
      return (
    ...
    case 'video':
      const getVideoUrl = () => {
        ...
        if (type === 'youtube') {
          ...
          return `https://www.youtube.com/embed/${id}?autoplay=${block.content.autoPlay ? 1 : 0}&loop=${block.content.loop ? 1 : 0}&mute=${block.content.muted ? 1 : 0}`;
        }
        if (type === 'vimeo') {
          const id = url.split('/').pop();
          return `https://player.vimeo.com/video/${id}?autoplay=${block.content.autoPlay ? 1 : 0}&loop=${block.content.loop ? 1 : 0}&muted=${block.content.muted ? 1 : 0}`;
        }
        return url;
      };
