    ...
    <div className="relative w-full h-[350px] sm:h-[500px] md:h-[600px] lg:h-[700px] overflow-hidden group bg-[#1d2327]">
      ...
      {slides.map((slide, index) => (
        ...
        >
          ...
          {/* Content */}
          <div className={cn(
            "relative h-full w-full px-5 sm:px-12 md:px-24 flex flex-col justify-center text-white overflow-hidden",
            // Desktop alignment
            slide.buttonPosition === 'center' ? 'md:items-center md:text-center' :
            slide.buttonPosition === 'right' ? 'md:items-end md:text-right' :
            'md:items-start md:text-left',
            // Mobile: force start alignment and prevent horizontal overflow
            "items-start text-left sm:items-center sm:text-center md:items-start md:text-left"
          )}>
            <div className={cn(
              "space-y-3 md:space-y-6 w-full max-w-[calc(100vw-40px)] md:max-w-4xl transform transition-all duration-700 delay-300",
              ...
              <span className="text-primary font-black text-[9px] md:text-xs uppercase tracking-[0.3em] block">
                {slide.subtitle}
              </span>
              <h2 className="text-2xl sm:text-4xl md:text-6xl font-black italic uppercase leading-[1.1] break-words">
                {slide.title}
              </h2>
              <p className="text-gray-200 text-[11px] sm:text-sm md:text-lg max-w-xl leading-relaxed line-clamp-3 md:line-clamp-none break-words">
                {slide.description}
              </p>
              <div className="pt-2 md:pt-4 flex flex-row items-center gap-3 md:gap-6 flex-wrap">
                <a 
                  href={slide.buttonLink}
                  className="inline-flex bg-primary text-white px-5 py-2.5 md:px-8 md:py-4 text-[10px] md:text-sm font-black uppercase tracking-widest hover:opacity-90 transition-all shadow-xl shadow-primary/30 items-center justify-center group/btn whitespace-nowrap"
                >
                  {slide.buttonText}
                  <ArrowRight className="w-3 h-3 md:w-4 md:h-4 ml-2 group-hover/btn:translate-x-1 transition-transform" />
                </a>
                {slide.secondButtonText && (
                  <a href={slide.secondButtonLink || '#'} className="text-white text-[10px] md:text-sm font-black uppercase tracking-widest hover:text-primary transition-colors border-b-2 border-white/20 hover:border-primary pb-1 whitespace-nowrap">
                    {slide.secondButtonText}
                  </a>
                )}
              </div>
